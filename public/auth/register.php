<?php
    ...
    if (isset($_POST['register'])) {
        ...
        $uname = mysqli_real_escape_string($db, trim(strip_tags($_POST['username'])));
        ...
        $result = mysqli_query($db, "SELECT * FROM users WHERE `username` = '$uname' ");
        if ($result) {
            ...
            elseif (mysqli_num_rows($result) < 1) {
                if ($pass == $cpass) {
                    $query = "INSERT INTO users(`fullname`, `username`, `physical_address`, `password`, `genderID`)
                    VALUES('$fullname', '$uname', '$paddress', '$pass', '$gender')";
